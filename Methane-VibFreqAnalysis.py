...
freqRaw = scf_wfn.frequency_analysis['omega'].data # this command is just to get you started!

# Eliminate imaginary parts of frequencies,
freqReal = np.real(freqRaw)

# round the frequencies (to the nearest whole number),
freqRound = freqReal.round()
# and extract only the *non-zero* frequencies
freqNonzero = freqRound[freqRound != 0]


# Determine the unique non-zero frequencies and
freqUnique, counts = np.unique(freqNonzero, return_counts=True)
...
freqCounts = np.column_stack((freqUnique, counts))
print(freqCounts)


# Save the NumPy array with frequency and count data
# to a text file with the header line: 'freq degen'
np.savetxt('methaneUniqueFreqCounts.txt', freqCounts, delimiter=' ', newline='\n', header='freq degen')
